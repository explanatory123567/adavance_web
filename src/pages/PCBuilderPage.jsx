...
import { RotateCcw, RotateCw, ZoomIn, ZoomOut, Move, CheckCircle2, AlertTriangle, Zap, ShoppingCart, Bookmark, Plus, Trash2, Cpu, Monitor, CircuitBoard, Layers, HardDrive, Box, Snowflake, Wind, X, Share2, Printer, Sliders, ArrowUpRight, Gauge, Activity } from 'lucide-react';
...
function AssemblyPart({ category, component }) {
  ...
  if (component.modelUrl) {
    return <GltfPart url={component.modelUrl} position={partPositions[category]} />;
  }
...
function PCAssemblyScene({ builderConfig, modelAssets, cameraView, zoomLevel, rotationAngle }) {
  ...
  return (
    <Canvas shadows camera={{ position: [4.2, 2.4, 5.4], fov: 42 }} className="pc-builder-canvas">
      ...
      <OrbitControls enablePan={false} minDistance={3.5} maxDistance={9} target={[0, 0, 0]} />
    </Canvas>
  );
}
export default function PCBuilderPage() {
  ...
  const [buildNameInput, setBuildNameInput] = useState('NextGear Studio Battlestation');
  const [saveModalOpen, setSaveModalOpen] = useState(false);
  const [printModalOpen, setPrintModalOpen] = useState(false);

  // Financial & Diagnostic calculations
  const installedSlots = Object.entries(builderConfig).filter(([_, comp]) => comp !== null);
  const totalPrice = installedSlots.reduce((sum, [_, comp]) => sum + (comp?.price || 0), 0);
  ...
  const wattagePercentage = Math.min(100, Math.round(totalWattage / psuWattage * 100));
  const averageScore = installedSlots.length > 0 ? Math.round(installedSlots.reduce((sum, [_, comp]) => sum + (comp?.performanceScore || 85), 0) / installedSlots.length) : 90;
  ...
  const handleAddToCartClick = () => {
    ...
    addCustomBuildToCart({
      ...
      components: Object.fromEntries(Object.entries(builderConfig).map(([k, v]) => [k, v ? v.name : 'None']))
    });
  };
  return <div className="pc-builder-page inline-pcbuilderpage-0">
      <div className="container-wide">
        ...
        <div className="flex-between inline-pcbuilderpage-1">
          ...
          <div className="inline-pcbuilderpage-5">
            <button onClick={() => setPrintModalOpen(true)} className="btn btn-secondary btn-sm inline-pcbuilderpage-6">
              <Printer size={14} /> Export Spec Sheet
            </button>
            <button onClick={() => showToast('Share Link Ready', 'Shareable build configuration copied.', 'success')} className="btn btn-secondary btn-sm inline-pcbuilderpage-7">
          ...
          <section className="cyber-card-static inline-pcbuilderpage-9">
            ...
            <div className="inline-pcbuilderpage-13">
              ...
              return <div key={cat.id} style={{
                background: comp ? 'rgba(255, 255, 255, 0.02)' : 'transparent',
                border: comp ? '1px solid var(--border-medium)' : '1px dashed var(--border-subtle)',
                ...
                transition: 'all 0.2s'
                    ...
                    <div className="flex-between inline-pcbuilderpage-14">
                      ...
                      {comp && <button onClick={() => clearBuilderSlot(cat.id)} title="Remove part" className="inline-pcbuilderpage-16">
                          <Trash2 size={12} />
                        </button>}
                    </div>
                    ...
                    {comp ? <div>
                        ...
                        <div className="flex-between inline-pcbuilderpage-18">
                          ...
                          <button onClick={() => setActiveSlotPicker(cat.id)} className="btn btn-secondary btn-sm inline-pcbuilderpage-20">
                            Swap
                          </button>
                        </div>
                      </div> : <button onClick={() => setActiveSlotPicker(cat.id)} className="inline-pcbuilderpage-21">
            ...
            <div className="cyber-card-static inline-pcbuilderpage-23">
              ...
              <div className="flex-between inline-pcbuilderpage-24">
                <div className="inline-pcbuilderpage-25">
                  <span className="inline-pcbuilderpage-26">
                    Camera:
                  </span>
                  {['front', 'inside', 'back', 'top'].map(preset => <button key={preset} onClick={() => handleCameraPreset(preset)} style={{
                  padding: '4px 10px',
                  borderRadius: '6px',
                  fontSize: '11.5px',
                  fontWeight: 600,
                  textTransform: 'capitalize',
                  background: cameraView === preset ? 'rgba(0, 212, 255, 0.15)' : 'rgba(255, 255, 255, 0.04)',
                  color: cameraView === preset ? 'var(--accent-cyan)' : 'var(--text-secondary)',
                  border: cameraView === preset ? '1px solid var(--accent-cyan)' : '1px solid transparent'
                }}>
                      {preset}
                    </button>)}
                </div>
              ...
              <div className="inline-pcbuilderpage-28 pc-assembly-viewport">
                ...
                <div className="inline-pcbuilderpage-29">
                  <button onClick={() => setRotationAngle(prev => prev - 30)} className="btn-icon inline-pcbuilderpage-30">
                    <RotateCcw size={13} />
                  </button>
                  <button onClick={() => setRotationAngle(prev => prev + 30)} className="btn-icon inline-pcbuilderpage-31">
                    <RotateCw size={13} />
                  </button>
                  <button onClick={() => setZoomLevel(prev => Math.min(1.5, prev + 0.15))} className="btn-icon inline-pcbuilderpage-32">
                    <ZoomIn size={13} />
                  </button>
                  <button onClick={() => setZoomLevel(prev => Math.max(0.7, prev - 0.15))} className="btn-icon inline-pcbuilderpage-33">
                    <ZoomOut size={13} />
                  </button>
                  <button onClick={() => {
                  setZoomLevel(1);
            ...
            <div className="cyber-card-static inline-pcbuilderpage-35">
              <div className="flex-between inline-pcbuilderpage-36">
                <div className="inline-pcbuilderpage-37">
                  ...
                  <h3 className="inline-pcbuilderpage-38">Real-Time Bottleneck Balance Meter</h3>
                </div>

                {/* Target Gaming Resolution Selector */}
                <div className="inline-pcbuilderpage-39">
                  {['1080p', '1440p', '4k'].map(res => <button key={res} onClick={() => setSelectedResolution(res)} style={{
                  padding: '3px 8px',
                  borderRadius: '4px',
                  fontSize: '11px',
                  fontWeight: 700,
                  textTransform: 'uppercase',
                  background: selectedResolution === res ? 'var(--accent-cyan)' : 'rgba(255, 255, 255, 0.04)',
                  color: selectedResolution === res ? '#070a12' : 'var(--text-muted)'
                }}>
                      {res}
                    </button>)}
                </div>
              ...
              <div className="inline-pcbuilderpage-40">
                <div className="flex-between inline-pcbuilderpage-41">
                  <span className="inline-pcbuilderpage-42">
                    Bottleneck Index: <strong className="inline-pcbuilderpage-43">{bottleneckData.percentage}%</strong>
                  </span>
                  <span className={`badge ${bottleneckData.tier === 'Optimal' ? 'badge-green' : 'badge-amber'}`} style={{
                  fontSize: '10px'
                }}>
                ...
                <div className="bottleneck-components">
                  {[['CPU', bottleneckData.cpuScore], ['GPU', bottleneckData.gpuScore]].map(([label, score]) => (
                    <div className="bottleneck-components__row" key={label}>
                      <span>{label}</span>
                      <div className="bottleneck-components__track">
                        <div style={{ width: `${score}%`, background: label === 'CPU' ? '#34d399' : '#60a5fa' }} />
                      </div>
                      <strong>{score}%</strong>
                    </div>
                  ))}
                ...
                <div className="inline-pcbuilderpage-45">
                  ...
                  background: bottleneckData.percentage < 8 ? 'var(--accent-emerald)' : 'var(--accent-amber)',
                  borderRadius: '4px',
          ...
          <section className="cyber-card-static inline-pcbuilderpage-47">
            ...
            {activeTab === 'summary' && <div>
                ...
                <div className="inline-pcbuilderpage-49">
                  <div className="inline-pcbuilderpage-50">Estimated Total Cost</div>
                ...
                <div style={{
              background: compatibilityStatus.isCompatible ? 'rgba(16, 185, 129, 0.08)' : 'rgba(239, 68, 68, 0.1)',
              border: compatibilityStatus.isCompatible ? '1px solid rgba(16, 185, 129, 0.3)' : '1px solid rgba(239, 68, 68, 0.3)',
              borderRadius: 'var(--radius-md)',
              padding: '12px',
              marginBottom: '16px'
            }}>
                  <div className="inline-pcbuilderpage-53">
                    {compatibilityStatus.isCompatible ? <CheckCircle2 size={16} color="#34d399" /> : <AlertTriangle size={16} color="#f87171" />}
                    <span style={{
                  fontSize: '12.5px',
                  fontWeight: 800,
                  color: compatibilityStatus.isCompatible ? '#34d399' : '#f87171'
                }}>
                      {compatibilityStatus.isCompatible ? '100% Real-Time Compatibility Verified' : 'Compatibility Issues Detected'}
                    </span>
                  </div>

                  <ul className="inline-pcbuilderpage-54">
                    {compatibilityStatus.passes.slice(0, 3).map((pass, idx) => <li key={idx}>✓ {pass}</li>)}
                    {compatibilityStatus.issues.map((err, idx) => <li key={idx} className="inline-pcbuilderpage-55">⚠️ {err.message}</li>)}
                  </ul>
                </div>
                ...
                <div className="inline-pcbuilderpage-56">
                  <div className="flex-between inline-pcbuilderpage-57">
                    ...
                    <strong className="inline-pcbuilderpage-59">{totalWattage}W / {psuWattage}W</strong>
                  </div>
                ...
                <div className="inline-pcbuilderpage-61">
                  <button onClick={handleAddToCartClick} className="btn btn-primary inline-pcbuilderpage-62">
                    <ShoppingCart size={15} />
                    <span>{userRole === 'guest' ? 'Login to Order Build' : 'Add Build to Cart'}</span>
                  </button>

                  <button onClick={handleSaveBuildClick} className="btn btn-secondary inline-pcbuilderpage-63">
                    <Bookmark size={15} />
                    <span>{userRole === 'guest' ? 'Login to Save Build' : 'Save Build'}</span>
                  </button>
                </div>
              </div>}

            {/* TAB 2: BUILD ANALYZER */}
            {activeTab === 'analyzer' && <div className="build-analyzer">
                <div className="build-analyzer__panel">
                  ...
                  <div className="build-analyzer__content">
                    <div className="build-analyzer__gauge-wrap">
                      <div className="build-analyzer__gauge" style={{ '--score': averageScore }}>
                        <div className="build-analyzer__gauge-inner">
                          <span className="build-analyzer__gauge-value">{averageScore}</span>
                          <span className="build-analyzer__gauge-max">/100</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="build-analyzer__summary">
                      <h4>GOOD BUILD</h4>
                      <p>Your build is well balanced. There are some areas you can improve.</p>
                    </div>
                  </div>
                    ...
                    {[
                      ...
                      <div key={label} className="build-analyzer__metric">
                        <div className="build-analyzer__metric-label">
                          <span className="build-analyzer__metric-icon" aria-hidden="true">
                            {label === 'Performance' ? '◌' : label === 'Value' ? '◍' : label === 'Upgradability' ? '↻' : label === 'Cooling' ? '◔' : label === 'Power Efficiency' ? '⚡' : '◌'}
                          </span>
                          <span>{label}</span>
                        </div>
            ...
            {activeTab === 'priorities' && <div>
                <h4 className="inline-pcbuilderpage-86">
                  <Sliders size={16} color="var(--accent-cyan)" /> Priority Weighting Sliders
                </h4>
                ...
                <div className="inline-pcbuilderpage-88">
                  <div className="flex-between inline-pcbuilderpage-89">
                    ...
                    <strong className="inline-pcbuilderpage-90">{priorityProfile.budget}%</strong>
                  </div>
                  <input type="range" min="0" max="100" value={priorityProfile.budget} onChange={e => setPriorityProfile(prev => ({
                ...prev,
                ...
                <div className="inline-pcbuilderpage-92">
                  <div className="flex-between inline-pcbuilderpage-93">
                    ...
                    <strong className="inline-pcbuilderpage-94">{priorityProfile.performance}%</strong>
                  </div>
                  <input type="range" min="0" max="100" value={priorityProfile.performance} onChange={e => setPriorityProfile(prev => ({
                ...prev,
                ...
                <div className="inline-pcbuilderpage-96">
                  <div className="flex-between inline-pcbuilderpage-97">
                    ...
                    <strong className="inline-pcbuilderpage-98">{priorityProfile.upgradability}%</strong>
                  </div>
                  <input type="range" min="0" max="100" value={priorityProfile.upgradability} onChange={e => setPriorityProfile(prev => ({
                ...prev,
                ...
                <div className="inline-pcbuilderpage-100">
                  ✓ Recommendation engine adjusted to prioritize long-life platforms & high-efficiency power supplies.
                </div>
              </div>}

            {/* TAB 4: FUTURE UPGRADES ADVISOR */}
            {activeTab === 'upgrades' && <div>
                ...
                <div className="inline-pcbuilderpage-102">
                  {futureUpgradesAdvice.map((adv, idx) => <div key={idx} className="inline-pcbuilderpage-103">
                      <div className="inline-pcbuilderpage-104">
                        {adv.title}
                      </div>
                      <div className="inline-pcbuilderpage-105">
                        {adv.desc}
                      </div>
                    </div>)}
                </div>
      ...
      {activeSlotPicker && <div className="modal-overlay" onClick={() => setActiveSlotPicker(null)}>
          <div className="modal-content modal-content-lg inline-pcbuilderpage-106" onClick={e => e.stopPropagation()}>
            <div className="flex-between inline-pcbuilderpage-107">
              <div>
                <span className="badge badge-cyan inline-pcbuilderpage-108">RECOMMENDATION ENGINE</span>
                <h3 className="inline-pcbuilderpage-109">
                  Select {activeSlotPicker.toUpperCase()} Hardware Match
                </h3>
              </div>
              <button onClick={() => setActiveSlotPicker(null)} className="btn-icon">
                <X size={16} />
              </button>
            </div>
            ...
            const isCurrent = builderConfig[activeSlotPicker]?.id === product.id;
            return <div key={product.id} className="cyber-card" style={{
              padding: '14px',
              display: 'flex',
              gap: '14px',
              borderColor: product.isRecommendedMatch ? 'var(--accent-cyan)' : 'var(--border-subtle)',
              background: product.isRecommendedMatch ? 'rgba(0, 212, 255, 0.04)' : 'rgba(16, 22, 36, 0.7)'
            }}>
                    <img src={product.image} alt={product.name} className="inline-pcbuilderpage-111" />
                    <div className="inline-pcbuilderpage-112">
                      {product.isRecommendedMatch && <div className="inline-pcbuilderpage-113">
                ...
                }} className={`btn ${isCurrent ? 'btn-secondary' : 'btn-primary'} btn-sm`} style={{
                  width: '100%',
                  padding: '5px'
                }}>
      ...
      {printModalOpen && <div className="modal-overlay" onClick={() => setPrintModalOpen(false)}>
          <div className="modal-content modal-content-lg inline-pcbuilderpage-117" onClick={e => e.stopPropagation()}>
            <div className="flex-between inline-pcbuilderpage-118">
              <div>
                <div className="inline-pcbuilderpage-119">NEXT GEAR BUILD SPECIFICATION</div>
                <h3 className="inline-pcbuilderpage-120">{buildNameInput}</h3>
              </div>
              <button onClick={() => setPrintModalOpen(false)} className="btn-icon">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="inline-pcbuilderpage-121">
              {CATEGORIES.map(cat => {
            const comp = builderConfig[cat.id];
            return <div key={cat.id} className="flex-between inline-pcbuilderpage-122">
            ...
            <div className="inline-pcbuilderpage-126">
              <div className="flex-between">
                <div>
                  <div className="inline-pcbuilderpage-127">Total Estimated Build Cost</div>
                  <div className="inline-pcbuilderpage-128">${totalPrice.toLocaleString()}</div>
                </div>
                <div className="inline-pcbuilderpage-129">
                  <span className="badge badge-green inline-pcbuilderpage-130">100% COMPATIBLE</span>
                  <div className="inline-pcbuilderpage-131">Power Draw: {totalWattage}W (Optimal Load)</div>
                </div>
              </div>
            </div>
            ...
            <div className="inline-pcbuilderpage-132">
              <button onClick={() => {
            window.print();
            showToast('PDF Exported', 'Cleanroom assembly spec sheet generated.', 'success');
            setPrintModalOpen(false);
          }} className="btn btn-primary inline-pcbuilderpage-133">
                <Printer size={15} /> Print / Save as PDF
              </button>
              <button onClick={() => setPrintModalOpen(false)} className="btn btn-secondary">
                Close
              </button>
            </div>
          </div>
      ...
      {/* ==================== SAVE BUILD MODAL ==================== */}
      {saveModalOpen && <div className="modal-overlay" onClick={() => setSaveModalOpen(false)}>
          <div className="modal-content inline-pcbuilderpage-134" onClick={e => e.stopPropagation()}>
            <div className="flex-between inline-pcbuilderpage-135">
              <h3 className="inline-pcbuilderpage-136">Save Build Configuration</h3>
              <button onClick={() => setSaveModalOpen(false)} className="btn-icon">
                <X size={16} />
              </button>
            </div>
            ...
            <form onSubmit={handleConfirmSave}>
              <div className="form-group">
                ...
                <input type="text" className="form-control" value={buildNameInput} onChange={e => setBuildNameInput(e.target.value)} placeholder="e.g. NextGear Esports Streamer" required />
              </div>

              <div className="inline-pcbuilderpage-137">
                <div>Mounted Slots: <strong>{installedSlots.length} Components</strong></div>
                <div>Estimated Total: <strong>${totalPrice.toLocaleString()}</strong></div>
              </div>

              <button type="submit" className="btn btn-primary inline-pcbuilderpage-138">
                Confirm & Save Build
              </button>
            </form>
          </div>
