...
import { Filter, Search, SlidersHorizontal, RotateCcw, Cpu, Monitor, CircuitBoard, Layers, HardDrive, Zap, Box, Snowflake, Wind } from 'lucide-react';
const iconMap = {
  Cpu: Cpu,
  Monitor: Monitor,
...
export default function ComponentsPage() {
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(product => {
      ...
      if (sortBy === 'score') return (b.performanceScore || 0) - (a.performanceScore || 0);
      return 0; // featured default
    });
  }, [products, selectedCategoryFilter, selectedBrands, priceMax, minRating, inStockOnly, performanceTier, searchQuery, sortBy]);
  return <div className="components-page inline-componentspage-0">
      <div className="container-wide">
        ...
        <div className="inline-componentspage-1">
          ...
          <p className="inline-componentspage-5">
            Explore certified hardware with verified benchmark scores, socket compatibility matrixes, and live stock tracking.
          </p>
        </div>

        {/* Category Horizontal Quick Strip */}
        <div className="inline-componentspage-6">
          <button onClick={() => setSelectedCategoryFilter('all')} style={{
          padding: '10px 18px',
          ...
          gap: '8px',
          whiteSpace: 'nowrap',
          background: selectedCategoryFilter === 'all' ? 'var(--neon-cyan)' : 'rgba(255, 255, 255, 0.05)',
          color: selectedCategoryFilter === 'all' ? '#070913' : 'var(--text-secondary)',
          border: selectedCategoryFilter === 'all' ? '1px solid var(--neon-cyan)' : '1px solid var(--border-subtle)',
          boxShadow: selectedCategoryFilter === 'all' ? '0 0 15px var(--neon-cyan-glow)' : 'none',
          transition: 'all 0.2s'
        }}>
          ...
          return <button key={cat.id} onClick={() => setSelectedCategoryFilter(cat.id)} style={{
            padding: '10px 18px',
            borderRadius: 'var(--radius-md)',
            fontSize: '13px',
            fontWeight: 700,
        ...
        <div className="inline-componentspage-7">
          ...
          <aside className="cyber-card-static inline-componentspage-8">
            <div className="flex-between inline-componentspage-9">
              ...
              <button onClick={resetFilters} className="inline-componentspage-11">
            ...
            <div className="form-group inline-componentspage-12">
              ...
              <div className="inline-componentspage-13">
                <input type="text" placeholder="e.g. 5090, 7800X3D, RGB..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="form-control inline-componentspage-14" />
                <Search size={14} color="var(--text-muted)" className="inline-componentspage-15" />
              </div>
            </div>
            ...
            <div className="form-group inline-componentspage-16">
              <div className="flex-between inline-componentspage-17">
                <label className="form-label inline-componentspage-18">Max Budget</label>
                <span className="inline-componentspage-19">
                  ₱{priceMax.toLocaleString()}
                </span>
              </div>
              <input type="range" min="30" max="2500" step="50" value={priceMax} onChange={e => setPriceMax(Number(e.target.value))} className="inline-componentspage-20" />
            ...
            <div className="form-group inline-componentspage-22">
              ...
              }].map(tier => <button key={tier.id} onClick={() => setPerformanceTier(tier.id)} style={{
                textAlign: 'left',
                padding: '7px 12px',
                borderRadius: '6px',
                fontSize: '12.5px',
                background: performanceTier === tier.id ? 'rgba(0, 240, 255, 0.12)' : 'rgba(255, 255, 255, 0.03)',
                color: performanceTier === tier.id ? 'var(--neon-cyan)' : 'var(--text-secondary)',
                border: performanceTier === tier.id ? '1px solid rgba(0, 240, 255, 0.3)' : '1px solid transparent',
            ...
            <div className="form-group inline-componentspage-24">
              ...
              <div className="inline-componentspage-25">
                ...
                return <label key={brand} style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: '8px',
                  fontSize: '12.5px',
                  color: isChecked ? 'var(--neon-cyan)' : 'var(--text-secondary)',
                  cursor: 'pointer',
                  padding: '4px 6px',
                  borderRadius: '4px',
                  background: isChecked ? 'rgba(0, 240, 255, 0.08)' : 'transparent'
                }}>
                      <input type="checkbox" checked={isChecked} onChange={() => toggleBrand(brand)} className="inline-componentspage-26" />
                      <span>{brand}</span>
                    </label>;
              })}
              </div>
            ...
            <div className="form-group inline-componentspage-27">
              ...
              <div className="inline-componentspage-28">
                ...
                rating: 4.9,
                label: '4.9★+'
              }].map(r => <button key={r.rating} onClick={() => setMinRating(r.rating)} style={{
                flex: 1,
                padding: '6px 8px',
                borderRadius: '6px',
                fontSize: '11.5px',
                fontWeight: 600,
                background: minRating === r.rating ? 'rgba(251, 191, 36, 0.15)' : 'rgba(255,255,255,0.03)',
                color: minRating === r.rating ? '#fbbf24' : 'var(--text-secondary)',
                border: minRating === r.rating ? '1px solid #fbbf24' : '1px solid var(--border-subtle)'
              }}>
                    {r.label}
                  </button>)}
              </div>
            ...
            <div className="inline-componentspage-29">
              <label className="inline-componentspage-30">
                ...
                <input type="checkbox" checked={inStockOnly} onChange={e => setInStockOnly(e.target.checked)} className="inline-componentspage-31" />
              </label>
            </div>
          </aside>
          ...
          <main>
            ...
            <div className="cyber-card-static inline-componentspage-32">
              <div className="inline-componentspage-33">
                Showing <strong className="inline-componentspage-34">{filteredProducts.length}</strong> items in catalogue
              </div>

              <div className="inline-componentspage-35">
                <span className="inline-componentspage-36">Sort By:</span>
                <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="form-control inline-componentspage-37">
            ...
            {/* Products Grid */}
            {filteredProducts.length === 0 ? <div className="cyber-card-static inline-componentspage-38">
                <SlidersHorizontal size={40} color="var(--text-muted)" className="inline-componentspage-39" />
                <h3 className="inline-componentspage-40">No Hardware Matches Your Filters</h3>
                <p className="inline-componentspage-41">
                  Try relaxing your price budget or clearing specific brand checklists.
                </p>
